...
`mall-gateway` 是 mall-cloud 微服务电商平台的 **统一入口网关**，基于 **Spring Cloud Gateway (WebFlux 响应式)** 构建。所有客户端请求首先到达网关，由网关完成鉴权、路由转发、跨域处理后，再分发到下游微服务。
